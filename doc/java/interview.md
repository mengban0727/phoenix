...
* 进程和线程
  ...
  分配资源基本单位，cpu调度基本单位
  一个进程可以有多个线程
  在java中多个线程共享堆和方法区，每个线程有自己的程序计数器，虚拟机栈和本地方法栈
  线程开销小
  ```
...
* 线程之间的通信
  ...
  ```java
  互斥量Mutex，比如java中的锁Synchronized
  事件通知wait和notify
  信号量Semaphore
  ```
...
1. 线程池的执行流程，核心线程数 > 队列最大容量，能达到核心线程数的并发度吗？（50个核心线程队列20大小并发度能否达到50）
...
3. 对象创建过程以及生命周期
   ...
   ```java
   类加载
   内存分配-空闲列表和指针碰撞，内存分配并发问题，cas重试，tlab
   初始化零值-对象实例化后不设置初始值也能使用
   设置对象头-类型指针，gc分代年龄，hashcode，锁状态标识，
   init方法
   垃圾回收-三色标记
   ```
